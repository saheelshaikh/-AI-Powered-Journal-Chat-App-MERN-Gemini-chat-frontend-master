...
export const ChatProvider = ({ children }) => {
  ...
  let lastRequestTimeRef  =  useRef(0);
const MIN_INTERVAL = 5000;

  async function fetchResponse() {
    const now = Date.now();
    if (now - lastRequestTimeRef  < MIN_INTERVAL) {
    toast.error("Please wait a few seconds before sending another request.");
    return;
  }
  ...
  async function createChat() {
    ...
    try {
      const { data } = await axios.post(
        `${server}/api/chat/new`,{ userId: userId },
        {
          headers: {
            token: localStorage.getItem("token"),
          },
